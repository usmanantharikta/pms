	...
	<div class="page-content-wrapper">
		<div class="page-content">
			...
			<div class="col-md-12 dkp-isian hide">
				<!-- material -->
				<div class="table_material portlet red-flamingo box full-height-content full-height-content-scrollable">
					<div class="portlet-title">
						<div class="caption">
				...
				<!-- ./end masterial -->
				<!-- BEGIN MANPOWER -->
				<div class="table_manpower portlet yellow-gold box full-height-content full-height-content-scrollable">
					<div class="portlet-title">
						<div class="caption">
				...
				<!-- ./END MANPOWER -->
				<!-- BEGIN MACHINERY  -->
				<div class="portlet yellow-lemon box full-height-content full-height-content-scrollable table_machinery">
					<div class="portlet-title">
						<div class="caption">
				...
				<!-- END MACHINERY -->
				<!-- MONEY  -->
				<div class="portlet green-jungle box full-height-content full-height-content-scrollable table_money">
					<div class="portlet-title">
						<div class="caption">
				...
				<!-- ./END MONEY -->
				<!-- BEGIN SUB CONT  -->
				<div class="portlet blue-hoki box full-height-content full-height-content-scrollable table_subcont">
					<div class="portlet-title" >
						<div class="caption">
				...
				<!-- /END SUB CONT -->
				<!-- BEGIN HSE  -->
				<div class="portlet purple-studio box full-height-content full-height-content-scrollable table_hse">
					<div class="portlet-title">
						<div class="caption">
				...
				<!-- ./END HSE -->
				<!-- BEGITN OTHER -->
				<div class="portlet grey-cascade box full-height-content full-height-content-scrollable table_other">
					<div class="portlet-title">
						<div class="caption">
				...
				{
				?>
				<div class="portlet blue box full-height-content full-height-content-scrollable action-portlet">
					<div class="portlet-title">
						<div class="caption">
						...
						<div class="form-actions">
							<div class="row">
								...
								<div class="col-md-6">
									<div class="row">
										<div class="col-md-offset-4 col-md-8">
											<button id="submit-all" onclick="sent()" type="submit" class="btn green">Approve</button>
											<button type="button" onclick="goBack()"class="btn default">Cancel</button>
										</div>
									</div>
								</div>
							...
							<div class="row">
								...
								<div class="col-md-12 ">
									<div class="form-group">
										...
										<div class="col-md-9">
											<input name="basic_price_budget" type="text" class="form-control" placeholder="Basic Price Budget">
											<span class="help-block">
											 </span>
										</div>
									</div>
...
<input type="hidden" name="stat_prog" value="<?php if(isset($status_progress)) echo $status_progress; ?>">
<!-- ./END MODAL -->
...
<script>
var flag=true;
var number_colum=''; // number  colom for hidden
var $table='';
var global_id_dkp='';
   ...
   "drawCallback": function( settings ) {
       ...
       if(api.data().length==0){
         ...
         $('.table_material').removeClass('hide');
			 ...
			 if(stat_prog=='approved'){
			 api.column( 1 ).visible( false );
			 $(".action-portlet").addClass('hide');
			 }else {
...
"drawCallback": function( settings ) {
    ...
    // Output the data for the visible rows to the browser's console
    if(api.data().length==0){
      $('.table_manpower').addClass('hide');
    }else{
      $('.table_manpower').removeClass('hide');
    }
  }
});
...
"drawCallback": function( settings ) {
    ...
    // Output the data for the visible rows to the browser's console
    if(api.data().length==0){
      $('.table_machinery').addClass('hide');
    }else{
      $('.table_machinery').removeClass('hide');
    }
  }
});
...
"drawCallback": function( settings ) {
    ...
    // Output the data for the visible rows to the browser's console
    if(api.data().length==0){
      $('.table_money').addClass('hide');
    }else{
      $('.table_money').removeClass('hide');
    }
  }
});
//subcont
...
"drawCallback": function( settings ) {
    ...
    // Output the data for the visible rows to the browser's console
    if(api.data().length==0){
      $('.table_subcont').addClass('hide');
    }else{
      $('.table_subcont').removeClass('hide');
    }
  }
	});
  ...
  "drawCallback": function( settings ) {
      ...
      // Output the data for the visible rows to the browser's console
      if(api.data().length==0){
        $('.table_hse').addClass('hide');
      }else{
        $('.table_hse').removeClass('hide');
      }
    }
		});
    ...
    "drawCallback": function( settings ) {
        ...
        // Output the data for the visible rows to the browser's console
        if(api.data().length==0){
          $('.table_other').addClass('hide');
        }else{
          $('.table_other').removeClass('hide');
        }
...
var stat_prog=$('[name="stat_prog"]').val();
console.log('status progress :'+stat_prog);
// if(stat_prog=='approved'){
// 	$('#table_material').DataTable().column( 0 ).visible( false );
//
// }else {
// 	$('#table_material').DataTable().column( 0 ).visible( true );
// }
...
function show_hide(){
	...
	function add(){
		...
		if(parseInt($qty_budget) > parseInt($qty)){
			Metronic.stopPageLoading();
			bootbox.alert({
				title: '<p class="text-danger">Error</p>',
	...
	function edit_item(id, colom, table){
			$.ajax({
					...
					success: function(data)
					{
						...
						//tempel to Form_
						Metronic.stopPageLoading();
						var name=['id_mat', 'id_dkp', 'id_material', 'material_dec', 'material_name', 'qty', 'unit', 'weight', 'remarks', ,'basic_price_budget', 'keterangan_project'];
						var n=name.length;
						for (var i = 0; i < name.length; i++) {
	...
	function sent(){
		...
		callback: function (result) {
				if(result){
                   ...
                   callback: function(){ window.location.href = "<?php echo site_url().'project/list_dkp_approval'?>"}
								});
							},
							error: function (jqXHR, textStatus, errorThrown)
							{
