...
<head>
<meta charset="utf-8"/>
<title>List BPMB Approval</title>
					...
					<div class="portlet light bordered ">
						<div class="portlet-title">
							<div class="caption font-green-haze">
								...
								<span class="caption-subject bold uppercase"> List BPMB Approval</span>
							</div>
							<div class="actions">
...
<div id="material" class="modal fade" tabindex="-1" aria-hidden="true">
	<div class="modal-dialog modal-full">
		<div class="modal-content">
			...
			<div class="modal-body">
				<!-- <div class="scroller" style="height:300px" data-always-visible="1" data-rail-visible1="1"> -->
					<form id="form-bpmb-edit" method="post" enctype="multipart/form-data" class="form-horizontal">
...
var $table='';
	...
	$('#listBPMBApprov').parent().addClass('active');
...
function cancel(id_bpmb){
  ...
  if(id_bpmb==0){
    ...
    callback: function (result) {
        if(result){
					Metronic.startPageLoading({animate: true});
					var url='<?php echo site_url().'/ppc/cancel/'?>'+id_bpmb;
					$.ajax({
							url : url,
							type: "POST",
							// data: formdata,
...
$("#form-filter").submit(function(event){
	...
	$.ajax({
		url: "<?php echo site_url('ppc/filter_new/approval'); ?>",
		type: 'POST',
		data: formData,
		dataType: "JSON",
		async: false,
...
function approve(id_bpmb)
{
  if(id_bpmb==0){
    id_bpmb=$('[name="bpmb_no"]').val();
  }
	bootbox.confirm({
		message: "Are you sure want approve BPMB with id "+id_bpmb+" ?",
		buttons: {
				confirm: {
						label: 'Yes',
						className: 'btn-success'
		...
		callback: function (result) {
				if(result){
					Metronic.startPageLoading({animate: true});
					var url='<?php echo site_url().'/ppc/approve_bpmb/'?>'+id_bpmb;
					$.ajax({
							url : url,
							type: "POST",
...
function show_bpmb(bpmb_no)
{
	...
	$.ajax({
			url : '<?php echo site_url().'ppc/get_items_bpmb/'?>',
			type: "POST",
