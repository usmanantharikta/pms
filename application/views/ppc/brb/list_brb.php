					...
					<div class="portlet light bordered ">
						<div class="portlet-title">
							<div class="caption font-green-haze">
								...
								<span class="caption-subject bold uppercase"> List BRB Approval </span>
							</div>
							<div class="actions">
...
<div id="brb-view" class="modal fade" tabindex="-1" aria-hidden="true">
	<div class="modal-dialog modal-full">
		<div class="modal-content">
            ...
            <input type="hidden" name="id_brb_master">
			...
			<div class="modal-footer">
				...
				<button type="button" onclick="save_brb()" class="btn green btn-save-edit">Save</button>
        <button type="button" onclick="cancel('0')" class="btn green btn-cancel">Cancel</button>
        <?php
...
var $table='';
	...
	$('#listBRB').parent().addClass('active');
...
function edit_bpmb(bpmb_no)
{
  ...
  $.ajax({
      ...
      success: function(data)
      {
        ...
        if(data.detail[3]=='cancel')
        {
          $(".btn-cancel .btn-approve").hide();
        }
        console.log(data);
        Metronic.stopPageLoading();
...
function show_brb(id)
{
  ...
  $.ajax({
      ...
      success: function(data)
      {
        ...
        $('[name="create_date"]').val(data.detail[2]);
        $('[name="id_brb_master"]').val(data.detail[4]);
        if(data.detail[3]=='cancel')
        {
          $(".btn-cancel").hide();
          $(".btn-approve").hide();
...
function save_brb()
{
  ...
  var formd = new FormData($("#form-bpmb-final")[0]); //get data
  $.ajax({
      url : '<?php echo site_url().'/ppc_brb/save_brb_edited'?>',
      type: "POST",
      // data: {'id_item': id,'colom':colom, 'table': table},
      data: formd,
      processData: false,
      ...
      success: function(data)
      {
        if(data.status){
          Metronic.stopPageLoading();
          bootbox.alert({
            title: '<p class="text-danger">Success</p>',
            message: 'Well Done!!!',
          });
          // $("#material").modal('hide');
          $("#brb-view").modal('hide');
