					...
					<div class="portlet light bordered ">
						<div class="portlet-title">
							<div class="caption font-green-haze">
								...
								<span class="caption-subject bold uppercase"> List BRB Approved</span>
							</div>
							<div class="actions">
									...
									<div class="row">
										<div class="col-md-6">
											<div class="form-group">
												...
												<div class="col-md-9">
													...
													<select name="no_sppr_bpmb" class="form-control">
														<option value=""></option>
														<?php
														foreach ($sppr as $key) {
										...
										<div class="col-md-6">
											<div class="form-group">
												...
												<div class="col-md-9">
                          <select id="dkp_id" name="dkp_id" class="form-control" id="form_control_1">
                            <option value=""></option>
                            <!-- <option value=""></option> -->
                            <?php
...
<div id="detail-bpmb" class="modal fade" tabindex="-1" aria-hidden="true">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
				<h4 class="modal-title">BPMB Detail</h4>
			</div>
			<div class="modal-body">
				<!-- <div class="scroller" style="height:300px" data-always-visible="1" data-rail-visible1="1"> -->
				<form id="form-bpmb-final" method="post" enctype="multipart/form-data" class="form-horizontal">
					<input type="hidden" name="bpmb_no">
					<div class="form-body">
						<!-- <h3 class="form-section">General Information</h3> -->
						<div class="row">
							<div class="col-md-7">
								<h4 class="text-center">BUKTI PEMAKAIAN BAHAN MATERIAL (BPMB)</h4>
							</div>
              <div class="col-md-5">
								<div class="col-md-12">
									<div class="form-group">
        ...
        <button type="button" onclick="cancel(0)" class="btn btn-warning btn-approve">Cancel</button>
        <!-- <button type="button" onclick="save_bpmb_edited()" class="btn green-jungle">Save</button> -->
...
<!-- ./END MODAL -->
<!-- BEGIN Modal -->
<div id="brb-view" class="modal fade" tabindex="-1" aria-hidden="true">
	<div class="modal-dialog modal-full">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
				<h4 class="modal-title">Create Bukti Retur Bahan</h4>
			</div>
			<div class="modal-body">
				<!-- <div class="scroller" style="height:300px" data-always-visible="1" data-rail-visible1="1"> -->
					<form id="form-bpmb-final" method="post" enctype="multipart/form-data" class="form-horizontal">
						...
						<div class="form-body">
							...
							<div class="row">
								<div class="col-md-6">
									<div class="form-group">
										<label class="control-label col-md-3">BRB NO</label>
										<div class="col-md-9">
											<input name="brb_no" type="text" class="form-control" placeholder="BRB NO">
											<span class="help-block">
											 </span>
										</div>
									</div>
                ...
                <div class="col-md-6">
                  <div class="form-group">
                    <label class="control-label col-md-3">SPbP NO</label>
                    <div class="col-md-9">
                      <input name="spbp_no" type="text" class="form-control" placeholder="SPbP NO">
                      <span class="help-block">
                       </span>
                    </div>
                  </div>
                ...
                <!-- /span -->
								...
								<div class="col-md-6 ">
									<div class="form-group">
										...
										<div class="col-md-9">
											<input name="dkp_no_bpmb" type="text" class="form-control" value="" placeholder="">
											<span class="help-block">
										</span>
										</div>
									</div>
								...
								<!--/span-->
								<div class="col-md-12">
									<table id="brb-table" class="table table-striped table-bordered table-hover">
									 <thead>
												<tr>
			...
			<div class="modal-footer">
				<button type="button" data-dismiss="modal" class="btn default">Close</button>
				<button type="button" onclick="save_brb()" class="btn green">Save</button>
			</div>
		</div>
	</div>
</div>
...
var $table='';
	...
	$('#brb').addClass('active open');
	$('#listBRBApproved').parent().addClass('active');
...
function cancel(id_brb){

  // if(id_brb==0){
  //   id_brb=$('[name="bpmb_no"]').val();
  // }
    ...
    callback: function (result) {
        if(result){
					Metronic.startPageLoading({animate: true});
					var url='<?php echo site_url().'/ppc_brb/cancel_brb/'?>'+id_brb;
					$.ajax({
							url : url,
							type: "POST",
							// data: formdata,
...
$("#form-filter").submit(function(event){
	...
	$.ajax({
		url: "<?php echo site_url('ppc_brb/get_brb_approved'); ?>",
		type: 'POST',
		data: formData,
		dataType: "JSON",
		async: false,
...
function approve(id_brb)
{
  // if(id_bpmb==0){
  //   id_bpmb=$('[name="bpmb_no"]').val();
  // }
	bootbox.confirm({
		message: "Are you sure want approve BPMB with id "+id_brb+" ?",
		buttons: {
				confirm: {
						label: 'Yes',
						className: 'btn-success'
		...
		callback: function (result) {
				if(result){
					Metronic.startPageLoading({animate: true});
					var url='<?php echo site_url().'/ppc_brb/approve_brb/'?>'+id_brb;
					$.ajax({
							url : url,
							type: "POST",
...
function save_bpmb_edited()
{
  ...
  $.ajax({
      url : '<?php echo site_url().'/ppc/save_bpmb_edited'?>',
      type: "POST",
...
function show_bpmb(bpmb_no)
{
	Metronic.startPageLoading({animate: true});
	console.log('BPMB NO: '+bpmb_no);
	$.ajax({
			url : '<?php echo site_url().'ppc_brb/get_brb_approval/'?>',
			type: "POST",
			data: {'bpmb_no': bpmb_no},
			// processData: false,
			// contentType: false,
			dataType: "JSON",
			success: function(data)
            ...
            { title: "KOMPONEN" },
            { title: "JENIS BAHAN" },
						{ title: "MATERIAL STANDARD (STOCK/MARKET)" },
						{ title: "QUANTITY" },
            { title: "UNIT" },
            ...
            { title: "DESCRIPTION" },
							...
							// { "width": "25%", "targets": 4 },
              {
				...
				$('[name="bpmb_no"]').val(data.detail[0]);
...
function edit_bpmb(bpmb_no)
{
  console.log(bpmb_no+ " was selected!! ");
  Metronic.startPageLoading({animate: true});
  $('.isi-table').html('');
  var formdata_ = new FormData($("#form_bpmb")[0]); //get data
  $.ajax({
      url : '<?php echo site_url().'/ppc/edit_bpmb'?>',
      type: "POST",
      data: {'bpmb_no': bpmb_no},
      // data: formdata_,
      // processData: false,
...
function save_bpmb_final()
{
  ...
  // $('.isi-table').html('');
  var formd = new FormData($("#form-bpmb-edit")[0]); //get data
  $.ajax({
      url : '<?php echo site_url().'/ppc/save_bpmb_edited'?>',
      type: "POST",
      // data: {'id_item': id,'colom':colom, 'table': table},
...
function show_brb(id)
{
  Metronic.startPageLoading({animate: true});
  $('.isi-table').html('');
  var formdata_ = new FormData($("#form_bpmb")[0]); //get data
  $.ajax({
      url : '<?php echo site_url().'/ppc_brb/get_brb_by_id/'?>'+id,
      type: "POST",
      // data: {'id_item': id,'colom':colom, 'table': table},
      data: formdata_,
      processData: false,
      ...
      success: function(data)
      {
        ...
        $('[name="material_id"]').val(data.material_no[2]);
        $("#brb-view").modal('show');
        $('#brb-table td:nth-child(2)').hide();
        $('[name="name_proj"]').val(data.customer);
        $('[name="dkp_id_bpmb"]').val(data.dkp[0]);
        $('[name="dkp_no_bpmb"]').val(data.dkp[1]);
        $('[name="nama_project"]').val(data.title);
        $('[name="brb_no"]').val(data.detail[0]);
        $('[name="spbp_no"]').val(data.detail[1]);
        $('[name="create_date"]').val(data.detail[2]);
